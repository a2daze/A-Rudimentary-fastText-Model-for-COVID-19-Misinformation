...
sites_filtered<-disinformation%>%filter(Primary_Language == 'English')
write.csv(sites_filtered, 'filtered_disinfo_full_withoutnarrative.csv')
...
for (i in 1:length(sites_filtered)) {
  tryCatch({
    ...
    message("Error with site: ", sites_filtered[i], " - Skipping to next site.")
      write_lines(text, "full_text_disinfo",append=TRUE)
  })
